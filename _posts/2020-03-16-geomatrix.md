...
description: GeoMatrix
img: GeoMatrix.png # Add image post (optional)
...
public static class GeometricProgressView extends View {
    ...
    private void initialize(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            figurePadding = DEFAULT_FIGURE_PADDING;
    ...
    private void buildFiguresUsingKites(List<PointF> angles, double startAngle, double distanceFromCenter) {
        for (int i = 0; i < angles.size(); i++) {
            ...
            figures.add(Aan.new Figure(path, color, isInEditMode() ? (int) (25.0 + i * (230.0 / numberOfAngles)) : 0));
        }
    }
...
//Example
GeometricProgressView a = new GeometricProgressView(this);
linear1.addView(a);
a.setNumberOfAngles(3);



__________________
//Custome
setNumberOfAngles(int);
setDuration(int);
setType(int); 0/1
setColor(int);
setFigurePadding(int);
```

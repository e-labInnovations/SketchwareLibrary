...
img: ShellRunner.png # Add image post (optional)
...
Shell.SU.run("su");
boolean root = true;
new RunCommandTask(root).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, _commands);

# User
boolean root = false;
new RunCommandTask(root).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, _commands);

# you need to get all printer
